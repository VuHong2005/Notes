...
function FolderNotes() {
    ...
    useEffect(() => {
        supabase.from("folders").select("*").eq("id", id).single().then(({ data }) => setFolder(data));
        supabase.from("notes").select("*").eq("folder_id", id).is("deleted_at", null)
    ...
    const filteredNotes = notes
        ...
        .sort((a, b) => {
            ...
            if (field === "title") { va = va.toLowerCase(); vb = vb.toLowerCase(); return sortDir === "asc" ? va.localeCompare(vb) : vb.localeCompare(va); }
    ...
    const renderListItem = (note) => {
        ...
        return (
            <div key={note.id} className="notes-list-item" style={{ cursor: "pointer" }} onClick={() => navigate(`/note/${note.id}`, { state: { folderId: id, folderName: folder?.name } })}>
                <div className="notes-item-body">
                    <div className="notes-item-title-row">
                        <span className="notes-item-title">{note.title || "Ghi chú mới"}</span>
                        <span className="notes-item-date">{formatDate(note.created_at)}</span>
    ...
    const renderGalleryItem = (note) => {
        ...
        return (
            <div key={note.id} className={`notes-gallery-card${checked ? " selected" : ""}`}
                onClick={() => selectMode ? toggleSelect(note.id) : navigate(`/note/${note.id}`, { state: { folderId: id, folderName: folder?.name } })}>
                {selectMode && (
                ...
                <div className="notes-gallery-footer">
                    ...
                    <div className="notes-gallery-date">{formatDate(note.created_at)}</div>
                </div>
            </div>
        );
    };
    ...
    return (
        <div className="notes-app">
                ...
                <div className="topbar-menu-wrap" ref={menuRef}>
                    ...
                    {showMenu && (
                        <div className="topbar-menu-dropdown">
                            ...
                            <div className="topbar-menu-divider" />
                            <button className="topbar-menu-item" onClick={() => setShowSortMenu(v => !v)}>
                            ...
                            <div className="topbar-menu-divider" />
                            <button className="topbar-menu-item" onClick={() => setShowGroupMenu(v => !v)}>
                                <div className="topbar-menu-item-sub">
                                    <span>Nhóm theo ngày</span>
                                    <span className="topbar-menu-sub-label">
                                        {GROUP_OPTIONS.find(o => o.key === groupByDate)?.label}
                                    </span>
                                </div>
                                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="20" height="20">
            ...
            <div className="notes-list-header">
                <h1>{folder?.name || "Thư mục"}</h1>
                {selectMode && (
                    <button className="select-cancel-btn" onClick={exitSelectMode}>Hủy</button>
                )}
            </div>
            ...
            {filteredNotes.length === 0 ? (
                <div className="notes-empty">{search ? "Không tìm thấy ghi chú" : "Thư mục trống"}</div>
            ) : (
                <>
                    ...
                    {unpinnedNotes.length > 0 && (
                        (isGrouped ? groupNotes(unpinnedNotes) : [{ label: null, items: unpinnedNotes }])
                        .map(({ label, items }) => (
            ...
            <div className="notes-bottombar">
                {selectMode ? (
                    ...
                    <>
                        ...
                        <button className="notes-compose-btn" onClick={() => navigate(`/add?folder_id=${id}`)}>
